...
type DeleteServicePackageDialogProps = {
    open: boolean;
...
export function DeleteServicePackageDialog({
    open,
    onOpenChange,
    package: pkg,
    packagesIndexPath,
}: DeleteServicePackageDialogProps) {
    ...
    const handleDelete = () => {
        router.delete(`${packagesIndexPath}/${pkg.id}`, {
            preserveScroll: true,
            ...
            <DialogContent className="border-content-border bg-card w-[calc(100%-1rem)] max-w-md sm:w-full">
                <DialogHeader>
                    <DialogTitle className="text-foreground">
                        Eliminar paquete «{pkg.name}»
                    </DialogTitle>
                    <DialogDescription>
                        ¿Está seguro de eliminar este paquete de servicio? Esta acción no se puede deshacer.
                    </DialogDescription>
                </DialogHeader>
